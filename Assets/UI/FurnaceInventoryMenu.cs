...
public class FurnaceInventoryMenu : ContainerInventoryMenu
{
    ...
    {
        if (((FurnaceInventory)inventory).getItem(((FurnaceInventory)inventory).getResultSlot()).material == Material.Air)
        {
            return;
        }
        if (((FurnaceInventory)inventory).getItem(((FurnaceInventory)inventory).getResultSlot()).material != pointerSlot.item.material && pointerSlot.item.material != Material.Air)
        {
            return;
        }

        pointerSlot.item.material = ((FurnaceInventory)inventory).getItem(((FurnaceInventory)inventory).getResultSlot()).material;
        pointerSlot.item.amount += ((FurnaceInventory)inventory).getItem(((FurnaceInventory)inventory).getResultSlot()).amount;
