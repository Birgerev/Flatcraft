...
public class Liquid : Block
{
    ...
    public void CheckSource()
    {
        ...
        var source = new Location(int.Parse(data.GetData("source_block").Split('.')[0]),
            int.Parse(data.GetData("source_block").Split('.')[1]));
        var sourceLocation = location + source;
    ...
    public void Flow()
    {
        ...
        if (liquidLevel > 1)
        {
            if (left.GetMaterial() == Material.Air)
                (location + new Location(-1, 0)).SetMaterial(GetMaterial())
                    .SetData(new BlockData("source_block=1.0,liquid_level=" + (liquidLevel - 1)));
